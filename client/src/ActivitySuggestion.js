...
export default function ActivitySuggestion() {
	...
	return (
		<>
			{!isLoading && <Card variant="outlined" sx={{ width: 640 }}>
				<Stack direction="column" alignItems="flex-start" sx={{ p: 1 }} spacing={1}>
					<Typography level="h2" fontSize="md">
						{ data.activity }
					</Typography>
					<Typography level="body2"><CategoryIcon sx={{ mr: 1 }} />{ data.type }</Typography>
				</Stack>
				<IconButton
					aria-label="refresh"
					variant="plain"
				...
				<AspectRatio minHeight="120px" maxHeight="200px" sx={{ my: 2 }}>
					<img
						src={`https://source.unsplash.com/random?${data.activity.split(' ', 5).join(",")}`}
						loading="lazy"
						alt=""
					/>
				</AspectRatio>
