...
const getTransformedActivity = async () => {
  ...
  if (lastUser) {
    ...
    let activity = await getActivity(accessibilityLevel, priceLevel);
  ...
  if (priceValue === PRICE.Free.max) {
    ...
    activityJson.price =
      priceValue <= PRICE.Low.max ? PRICE.Low.name : PRICE.High.name;
  }

  return activityJson;
};

export default getTransformedActivity;
